...
import type { PetShareCard } from "../types";

// Casi duplicado de ShareSheet.tsx apuntando a los endpoints de mascotas
// (sección 52 del documento fuente sugiere este componente separado) — a
// diferencia de reportes, acá TODOS los estados generan imagen (mascotas no
// tiene eje de confianza en Fase 1), así que el caso "solo enlace, sin
// imagen" nunca debería verse en la práctica, pero se deja el fallback igual
// por si la subida a almacenamiento falla.
export function PetShareSheet({ petId, open, onClose }: { petId: string; open: boolean; onClose: () => void }) {
  const [card, setCard] = useState<PetShareCard | null>(null);
  ...
  function track(channel: "whatsapp" | "web_share" | "copy_link" | "save_image") {
    api.recordPetShareEvent(petId, channel).catch(() => {});
  }
  ...
  async function shareNative() {
    ...
    try {
      if (card.imageUrl && navigator.canShare) {
        ...
        const file = new File([blob], "aquiayudamosve-mascota.png", { type: "image/png" });
  ...
  async function saveImage() {
    ...
    a.download = "aquiayudamosve-mascota.png";
    a.target = "_blank";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60" onClick={onClose}>
      ...
      >
        ...
        <h2 className="text-sm font-bold uppercase tracking-wide text-slate-400">Compartir mascota</h2>
          ...
          <>
            {card.imageUrl ? (
              ...
              <div className="mt-3 rounded-xl border border-warn px-3 py-2 text-xs text-warn">
                ⚠️ No se pudo generar la imagen. Se compartirá solo el enlace.
              </div>
            )}
