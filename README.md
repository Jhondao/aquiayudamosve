...
- **Mascotas perdidas/encontradas/heridas** (`/mascotas`, módulo comunitario de mascotas completo — Fases 1 a 4): reportar en menos de un minuto **sin cuenta** (nombre + correo o celular, foto en el mismo formulario), con la distinción "la vi" vs. "la tengo resguardada" (esta última nunca expone la dirección exacta), estado de ciclo de vida, mapa propio, y pieza para compartir por WhatsApp con cierre automático "🎉 ya volvió a casa". Confirmar/marcar incorrecto y reportar avistamientos ("LA VI AQUÍ") tampoco piden cuenta; un panel de "posibles coincidencias" sugiere mascotas de la misma especie reportadas cerca (mismo motor que usa moderación para detectar duplicados). **Revelar contacto**: con una advertencia explícita, muestra el correo/celular de quien reportó — decisión de producto para no construir mensajería interna, con un límite de tasa propio que nunca se salta con sesión activa. Directorio **"quiero ayudar con mascotas"** (`/mascotas/ayudar`, requiere cuenta para publicar): veterinarios, transporte, hogares temporales, puntos de atención y rescate, sugeridos automáticamente en reportes que necesitan esa categoría de ayuda. Cambiar el estado sigue siendo comunitario (cualquiera con cuenta, no solo quien reportó), con rastro en el log de auditoría — incluida la fusión de reportes duplicados, que un moderador hace desde `/admin` ([backend/src/modules/pets/](backend/src/modules/pets/), [frontend/src/pages/PetsPage.tsx](frontend/src/pages/PetsPage.tsx))
